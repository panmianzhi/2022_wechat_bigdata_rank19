...
class LXRTDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        is_matched = torch.tensor(is_matched, dtype=torch.float)
    ...
    def get_visual_feats(self, worker_id, idx: int) -> tuple:
        ...
        assert feat_dim == 512, "not the same video dim!"
    ...
    def random_feat(self, worker_id, feats):
        ...
        for i in range(len(feats)):
            prob = random.random()
            if prob < self.args.mask_ratio * 0.9:
                mask_feats[i, :] = 0.
                '''
